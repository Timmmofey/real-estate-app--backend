...
namespace GeoService.Application.Services
{
    public class GeoapifyGeoService : IGeoapifyGeoService
    {
        ...
        // Относительно нормальная версия, пусть будет на всякий случай
        //public async Task<IReadOnlyList<SettlementSuggestionDto>> GetSettlementSuggestionsAsync(
        //    string countryCode,
        //    string regionCode,
        //    string settlement
        ...
        )
        {
            ...
            // 1️⃣ собираем ВСЕ ключи (точно такие же, как раньше)
            var aliasKeysToTry = new List<string>
            {
                $"{aliasPrefix}:{normalizedQuery}" // exact
            };

            for (int len = normalizedQuery.Length - 1; len >= AliasMinPrefixLen; len--)
            {
                aliasKeysToTry.Add(
                    $"{aliasPrefix}:{normalizedQuery.Substring(0, len)}"
                );
            }

            // 2️⃣ ОДИН batched GET (MGET)
            var aliasValues = await _redis.GetManyAsync(aliasKeysToTry);

            // 3️⃣ берём первый непустой результат (длинный префикс → короткий)
            foreach (var aj in aliasValues)
            {
                if (!string.IsNullOrWhiteSpace(aj))
                {
                    baseKeys = JsonSerializer.Deserialize<List<string>>(aj, _jsonOptions)
                               ?? new List<string>();
                    break;
                }
            }
            ...
            {
                var distinctBaseKeys = baseKeys.Distinct().Take(500).ToList(); // cap to avoid huge pulls
                var baseJsons = await _redis.GetManyAsync(distinctBaseKeys);
